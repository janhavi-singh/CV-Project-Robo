...
def is_present(points, c):
    for pts in points:
        if c[0]<max(pts[0]) and c[0]>min(pts[0]):
            if c[1]<max(pts[1]) and c[1]>min(pts[1]):
                return True
    return False
...
def create_mask(image, hsv, lower, upper, center, key):
    ...
    for lv, uv in zip(lower, upper):
        ...
        for cnt in contours:
            if cv2.contourArea(cnt) > 600 and cv2.contourArea(cnt) < max_area:
                ...
                if dim[0]/dim[1] > 0.8 and dim[0]/dim[1] < 1.2:
...
file = open("C:\\Users\\Janhavi Singh\\Desktop\\opencv rubik's cube project\cube.txt", 'w')
for image in os.listdir("C:\\Users\\Janhavi Singh\\Desktop\\opencv rubik's cube project\\input_images"):
    img = cv2.imread("C:\\Users\\Janhavi Singh\\Desktop\\opencv rubik's cube project\\input_images" + "\\" + image)
    print(image)
    ...
    for i, c in enumerate(center):
        try:
            ...
            appeared.append(c['k'])
            order[i] = appeared.index(c['k'])+1
    print(order.reshape(n, n))
    for i in range(len(order)):
        file.write(str(int(order[i])) + ' ')
        if not (i+1) % 3:
            file.write(str('\n'))
